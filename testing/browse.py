...
class SeleniumWrapper:
        ...
        try:
            ...
            for element in elements:
                ...
                if (
                    ...
                    and (text == button_text or (button_text in text and abs(len(text) - len(button_text)) < 50))
                ):
            ...
            if before_content == after_content:
                output = "Clicked interactable element but nothing changed on the website."
            else:
    ...
    def fill_out_form(self, form_input: Optional[str] = None, **kwargs: Any) -> str:
        ...
        if form_input and type(form_input) == str:
            # Clean up form input
            form_input_str = truncate_string_from_last_occurrence(string=form_input, character="}")  # type: ignore
            try:
...
class ScrollInput(BaseModel):
    ...
    direction: str = Field(default="down", description="direction to scroll, either 'up' or 'down'")
